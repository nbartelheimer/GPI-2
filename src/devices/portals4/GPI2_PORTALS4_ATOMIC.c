/*
Copyright (c) Goethe University Frankfurt - Niklas Bartelheimer
...
pgaspi_dev_atomic_fetch_add(gaspi_context_t* const gctx,
  ...
  ret = PtlFetchAtomic(dev->group_atomic_md_h, gctx->nsrc.data.addr,
                       dev->group_atomic_md_h, (ptl_size_t)&val_add,
...
pgaspi_dev_atomic_compare_swap(gaspi_context_t* const gctx,
  ...
  ret = PtlSwap(dev->group_atomic_md_h, gctx->nsrc.data.addr,
                dev->group_atomic_md_h, (ptl_size_t)&val_new,
                sizeof(gaspi_atomic_value_t),
                dev->remote_info[rank].phys_address, dev->data_pt_idx, 0,
