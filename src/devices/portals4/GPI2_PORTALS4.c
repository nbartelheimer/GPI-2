...
pgaspi_dev_init_core(gaspi_context_t* const gctx) {
  ...
  void* ptr = malloc(passive_transfer_size_max);
  if(ptr == NULL) {
    return GASPI_ERR_MEMALLOC;
  }

  ret = posix_memalign((void**)ptr, page_size, passive_transfer_size_max);
  if(ret != 0) {
    return GASPI_ERROR;
  }
  ...
  ret =
      PtlLEAppend(portals4_dev_ctx->ni_h, portals4_dev_ctx->passive_comm_pt_idx,
                  &passive_le, PTL_PRIORITY_LIST, NULL,
  ...
  ret =
      PtlCTAlloc(portals4_dev_ctx->ni_h, &portals4_dev_ctx->passive_comm_ct_h);
  ...
  ret = PtlMDBind(portals4_dev_ctx->ni_h, &md,
                  &portals4_dev_ctx->passive_comm_md_h);
...
pgaspi_dev_comm_queue_delete(gaspi_context_t const* const gctx,
  ...
  ret = PtlEQFree(dev->comm_notif_err_eq_h[id]);
...
pgaspi_dev_comm_queue_create(const gaspi_context_t* const gctx,
  ...
  ret = PtlEQAlloc(dev->ni_h, gctx->config->queue_size_max,
                   &dev->comm_notif_err_eq_h[id]);
...
pgaspi_dev_comm_queue_is_valid(gaspi_context_t const* const gctx,
  ...
  if(PtlHandleIsEqual(portals4_dev_ctx->comm_notif_ct_h[id],
                      PTL_INVALID_HANDLE) &&
     PtlHandleIsEqual(portals4_dev_ctx->comm_notif_err_eq_h[id],
                      PTL_INVALID_HANDLE)) {
